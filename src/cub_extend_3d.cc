...
{
	ifstream ifs("/home/liuqiao/project_vscode/spectral-main/src/c1.txt");
	...
	std::cout << "lessgoo\n";
	PiecewiseJerkSpeedProblem piecewise_jerk_problem_0 = PiecewiseJerkSpeedProblem(num_of_knots, delta_t, init_s,init_s,num_of_obs);
	PiecewiseJerkSpeedProblem piecewise_jerk_problem_1 = PiecewiseJerkSpeedProblem(num_of_knots, delta_t, init_s,init_s,num_of_obs);
	...
	piecewise_jerk_problem_0.CorridorGeneration();
	piecewise_jerk_problem_0.PrintCorridor();
	std::cout<<"after corridor print"<<std::endl;
	piecewise_jerk_problem_0.CollisionCheck();
	std::cout<<"after CollisionCheck"<<std::endl;
	...
	std::cout<<"after set QP problem"<<std::endl;
	...
	for (int i = 0; i < num_of_knots && is_verbose; ++i) {
		...
		std::cout << std::fixed << std::setprecision(3) << "For t[" << i * delta_t << "], opt = " << s[i] << ", " << ds[i] << ", " << dds[i] << ", " << ddds << endl;
	}
	...
	ofstream ofs("/home/liuqiao/project_vscode/spectral-main/src/cuboid_slt.txt");
	if (!ofs.is_open()) {
		cout << "Not Found" << endl;
		return 1;
	}

...
namespace navigation
{
	...
	void PiecewiseJerkSpeedProblem::CollisionCheck()
	{
		...
		std::cout<<"total_num_corridors" << total_num_corridors;
		// waypoint p1;
		// p1.s = 1.0/delta_s_;	p1.l = 2.0; p1.t = 1.5/0.1;
		// traj.push_back(p1);
		std::vector<Cube> temp;
		cout <<"1temp.size()" << temp.size()<<std::endl;
		...
		std::cout << "new_corridor.size()"<<new_corridor.size();
	}
	...
	{	
		...
		for(int i=0; i<corridor.size(); i++)	std::cout << "\t"<<corridor[i].t<<"\n";
		std::cout << "1corridors.size()" << corridors.size() <<std::endl;
		//corridors.push_back(corridor);
		std::cout << "2corridors.size()" << corridors.size() <<std::endl;
		corridor.clear();
	}
